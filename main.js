class main {
    ...
    parseCSV(csvText) {
        ...
        lines.slice(1).forEach(line => {
            ...
            // Create Element object using only the constructor parameters
    ...
    parseFormula(formula) {
        // Simple parser for chemical formulas like H2O, NaCl, etc.
        const elements = {};
...
// Load element data from public JSON
async function loadElementData() {
    // Prefer local CSV (PToE.csv) because it includes valence electrons + specific heat.
    try {
        ...
        for (const line of lines.slice(1)) {
            ...
            elementData[symbol] = {
                ...
                // These two were showing null before; ensure numeric or null.
    ...
    // Fallback: Load element data from public JSON
    try {
        ...
        data.elements.forEach(el => {
            ...
            // Derive valence electrons if not present
...
chart = new Chart(ctx, {
    type: 'line',
    data: {
        labels: Array.from({length: 11}, (_, i) => i * 10), // 0, 10, 20, ..., 100
        datasets: []
    },
    options: {
        responsive: true,
